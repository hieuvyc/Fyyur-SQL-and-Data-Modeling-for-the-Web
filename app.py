...
def venues():
  ...
  for city, state in cities_states:
    city_venues = [venue for venue in venues if venue.city == city and venue.state == state]
    data.append({
      "city": city,
      "state": state,
      "venues": [{"id": venue.id, "name": venue.name,
                  "num_upcoming_shows": len([show for show in venue.shows if show.start_time > datetime.now()])} for
                 venue in city_venues]
    })
  # data_bk=[{
  #   "city": "San Francisco",
  #   "state": "CA",
...
def search_venues():
  ...
  response = {
    ...
    "data": [{"id": venue.id, "name": venue.name,
              "num_upcoming_shows": len([show for show in venue.shows if show.start_time > datetime.now()])} for venue
             in venues]
  }
...
def show_venue(venue_id):
  ...
  # If the venue doesn't exist, handle it gracefully
  if not venue:
    flash(f"Venue with ID {venue_id} does not exist.")
    return redirect(url_for('index'))

  # Get current date to separate past and upcoming shows
  now = datetime.now()

  # Retrieve past and upcoming shows
  past_shows = []
  upcoming_shows = []
...
def delete_venue(venue_id):
  ...
  try:
    ...
    # Handle the case where the venue does not exist
    if not venue:
      flash(f"Venue with ID {venue_id} does not exist.")
...
def artists():
  ...
  # Format the data to match the template's requirements
...
def search_artists():
  ...
  # Perform case-insensitive search on Artist names
...
def show_artist(artist_id):
  ...
  # Fetch the artist's past and upcoming shows
  past_shows = []
  upcoming_shows = []

  for show in artist.shows:  # Assuming Artist has a relationship with Show
    show_data = {
...
def edit_artist(artist_id):
  ...
  # Create an ArtistForm instance and populate it with artist data
  form = ArtistForm(
    name=artist.name,
    genres=artist.genres.split(','),  # Assuming genres are stored as a comma-separated string
    city=artist.city,
  ...
  # Prepare the artist data to pass to the template
  artist = {
    "id": artist.id,
    "name": artist.name,
    "genres": artist.genres.split(','),  # Convert comma-separated string to list
    "city": artist.city,
    "state": artist.state,
    "phone": artist.phone,
    "website": artist.website,
...
def edit_artist_submission(artist_id):
  ...
  try:
    ...
    # Update the artist's attributes with form data
    artist.name = request.form.get('name', artist.name)
    artist.genres = ','.join(request.form.getlist('genres')) if request.form.getlist('genres') else artist.genres
...
def edit_venue(venue_id):
  ...
  # Create a VenueForm instance and populate it with venue data
  form = VenueForm(
    name=venue.name,
    genres=venue.genres.split(','),  # Assuming genres are stored as a comma-separated string
    address=venue.address,
  ...
  # Prepare venue data to pass to the template
  venue = {
    "id": venue.id,
    "name": venue.name,
...
def edit_venue_submission(venue_id):
  ...
  try:
    ...
    # Update the venue's attributes with form data
    venue.name = request.form['name']
    venue.genres = ','.join(request.form.getlist('genres'))  # Convert list to comma-separated string
    ...
    # Rollback in case of an error
    db.session.rollback()
    flash(f"An error occurred. Venue could not be updated. Error: {str(e)}")
  finally:
    # Close the session
    db.session.close()
...
def create_artist_submission():
  ...
  # TODO: modify data to be the data object returned from db insertion
  try:
    # Create a new Artist object with data from the form
    new_artist = Artist(
      name=request.form['name'],
      city=request.form['city'],
      state=request.form['state'],
      phone=request.form['phone'],
      genres=request.form.getlist('genres'),  # Multiple selection for genres
      image_link=request.form['image_link'],
      facebook_link=request.form['facebook_link'],
      website=request.form['website_link'],
      seeking_venue=request.form.get('seeking_venue') == 'y',  # Checkbox for seeking_venue
      seeking_description=request.form['seeking_description']
    )
